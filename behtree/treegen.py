...
class Condition(object):
	...
	def generate(self, blackboard):
		...
		self.var = random.choice(blackboard["metrics"])
		self.op = random.choice(['<','>'])
		if self.var == 'centerx':
			self.value = random.choice(blackboard["dimx"])
		if self.var == 'centery':
			self.value = random.choice(blackboard["dimy"])
		if self.var == 'agentsx':
			self.value = random.choice(blackboard["dimx"])
		if self.var == 'agentsy':
			self.value = random.choice(blackboard["dimy"])
		...
		if self.var == 'lowery':
			self.value = random.choice(blackboard["dimy"])
		if self.var == 'density':
			self.value = random.choice(blackboard["density"])	
		if self.var == 'coverage':
			self.value = random.choice(blackboard["coverage"])
		if self.var == 'belief':
			self.value = random.choice(blackboard["belief"])
...
class Action(object):
	...
	def generate(self, blackboard):
		...
		self.type = random.choice(blackboard["actions"])
		return self.type
...
class Param(object):
	...
	def generate(self, blackboard):
		...
		if self.type == 'random':
			self.param = random.choice(blackboard["randparam"])
		elif self.type == 'rot_anti':
			self.param = random.choice(blackboard["rotparam"])
		elif self.type == 'rot_clock':
			self.param = random.choice(blackboard["rotparam"])
		else:
			self.param = random.choice(blackboard["dirparam"])
		return self.type
...
class Env_control(object):
	...
	def generate(self, blackboard):
		...
		self.type = random.choice(blackboard["envcontrol"])

		self.pos[0] = random.choice(blackboard["envcontrol_x"])
		self.pos[1] = random.choice(blackboard["envcontrol_y"])
		return self.type
...
class Operator(object):
	...
	def generate(self, blackboard):
		...
		self.type = random.choice(blackboard["operators"])
		self.size = random.choice(blackboard["opsize"])
...
class individual(object):
	...
	def copy(self):
		...
		newind.position = self.position
		newind.neighbours = self.neighbours[:]
...
class tree(object):
	...
	def make_tree(self, height, blackboard):
		...
		while depth <= height:
			...
			if depth == 0:
				...
				choice = random.choice(blackboard["act_types"])
			...
			if n == 0:
				...
				# Operators
					...
					if type(ind.genome[n]) is Condition:
						...
						if ind.genome[n].var == 'agentsy':
							name = 'AgentsY'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessAgenty()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterAgenty()
			
						if ind.genome[n].var == 'agentsx':
							name = 'CenterX'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessAgentx()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterAgentx()

						if ind.genome[n].var == 'mediany':
							name = 'MedianY'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessMediany()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterMediany()
			
						if ind.genome[n].var == 'medianx':
							name = 'MedianX'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessMedianx()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterMedianx()

						if ind.genome[n].var == 'uppery':
							name = 'UpperY'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessUpperY()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterUpperY()
			
						if ind.genome[n].var == 'upperx':
							name = 'UpperX'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessUpperX()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterUpperX()

						if ind.genome[n].var == 'lowery':
							name = 'LowerY'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessLowerY()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterLowerY()
			
						if ind.genome[n].var == 'lowerx':
							name = 'LowerX'
							if ind.genome[n].op == '<':
								node = tree_nodes.LessLowerX()
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterLowerX()
						...
						if ind.genome[n].var == 'belief':
							name = 'Belief'
							if ind.genome[n].op == '>':
								node = tree_nodes.GreaterBelief()
							if ind.genome[n].op == '<':
								node = tree_nodes.LessBelief()
