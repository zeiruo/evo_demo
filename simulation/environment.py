...
class double_entrance(object):
    ...
    def __init__(self, h, w, doorwidth, orient, origin):
        ...
        if orient == 'top':
            ...
            # Bottom doorway
            self.walls.append(wall())
            self.walls[2].start = [origin[0]-(0.5*w), origin[1]-(0.5*h)]; self.walls[2].end = [origin[0]-(0.5*doorwidth), origin[1]-(0.5*h)]

            self.walls.append(wall())
            self.walls[3].start = [origin[0]+(0.5*doorwidth), origin[1]-(0.5*h)]; self.walls[3].end = [origin[0]+(0.5*w), origin[1]-(0.5*h)]

            # top doorway 
            self.walls.append(wall())
            self.walls[4].start = [origin[0]-(0.5*w), origin[1]+(0.5*h)]; self.walls[4].end = [origin[0]-(0.5*doorwidth), origin[1]+(0.5*h)]

            self.walls.append(wall())
            self.walls[5].start = [origin[0]+(0.5*doorwidth), origin[1]+(0.5*h)]; self.walls[5].end = [origin[0]+(0.5*w), origin[1]+(0.5*h)]
        ...
        if orient == 'side':
            ...
            self.walls.append(wall())
            self.walls[2].start = [origin[0]-(0.5*w), origin[1]+(0.5*h)]; self.walls[2].end = [origin[0]-(0.5*w), origin[1]+(0.5*doorwidth)]

            self.walls.append(wall())
            self.walls[3].start = [origin[0]-(0.5*w), origin[1]-(0.5*doorwidth)]; self.walls[3].end = [origin[0]-(0.5*w), origin[1]-(0.5*h)]

            self.walls.append(wall())
            self.walls[4].start = [origin[0]+(0.5*w), origin[1]+(0.5*h)]; self.walls[4].end = [origin[0]+(0.5*w), origin[1]+(0.5*doorwidth)]

            self.walls.append(wall())
            self.walls[5].start = [origin[0]+(0.5*w), origin[1]-(0.5*doorwidth)]; self.walls[5].end = [origin[0]+(0.5*w), origin[1]-(0.5*h)]
...
class diag(object):
    ...
    def __init__(self, start, end, resolution):
        ...
        for n in range(resolution):
            ...
            self.walls.append(wall())
            self.walls[n].start = [start[0] + (n*diffx/resolution), start[1] + (n*diffy/resolution)]
            self.walls[n].end = [start[0] + ((n+1)*diffx/resolution) , start[1] + (n*diffy/resolution)]
        length = len(self.walls)
          # make horizontal lines
        for n in range(resolution):

            self.walls.append(wall())
            self.walls[n+ length].start = [start[0] + ((n+1)*diffx/resolution), start[1] + (n*diffy/resolution)]
            self.walls[n+ length].end = [start[0] + ((n+1)*diffx/resolution) , start[1] + ((n+1)*diffy/resolution)]
